    ...
    | Some command ->
        if String.length command >= 6 && String.sub command 0 6 = "rm -rf" then begin
          ...
          let output = Claude.Hooks.PreToolUse.deny
            ~reason:"Command contains dangerous 'rm -rf' pattern" () in
          Claude.Hooks.continue
            ~system_message:"Blocked dangerous rm -rf command"
            ~hook_specific_output:(Claude.Hooks.PreToolUse.output_to_json output)
            ()
        end else
          Claude.Hooks.continue ()
    | _ ->
...
let run_example ~sw ~env =
  ...
  let client = Claude.Client.create ~options ~sw ~process_mgr:env#process_mgr () in
