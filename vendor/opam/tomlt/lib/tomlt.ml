...
module Tz = struct
  ...
  let compare a b = match a, b with
    ...
    | Offset _, UTC -> 1
    | Offset { hours = h1; minutes = m1 }, Offset { hours = h2; minutes = m2 } ->
        Int.compare h1 h2 <?> lazy (Int.compare m1 m2)
...
module Date = struct
  ...
  let compare a b =
    ...
    <?> lazy (Int.compare a.month b.month)
    <?> lazy (Int.compare a.day b.day)
...
module Time = struct
  ...
  let compare a b =
    ...
    <?> lazy (Int.compare a.minute b.minute)
    <?> lazy (Int.compare a.second b.second)
    <?> lazy (Float.compare a.frac b.frac)
...
module Datetime = struct
  ...
  let compare a b =
    ...
    <?> lazy (Time.compare a.time b.time)
    <?> lazy (Tz.compare a.tz b.tz)
...
module Datetime_local = struct
  ...
  let compare a b =
    Date.compare a.date b.date <?> lazy (Time.compare a.time b.time)
...
let datetime_string = {
  ...
  doc = "";
  dec = (function
    | Toml.Datetime s | Toml.Datetime_local s
    | Toml.Date_local s | Toml.Time_local s -> Ok s
    | v -> Error (Type_mismatch { expected = "datetime"; got = type_name v }));
