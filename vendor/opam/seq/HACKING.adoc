...
2. Consult xref:INSTALL.adoc[] for build instructions. Here is the gist of it:
+
[source,sh]
----
./configure
...
link:https://gallium.inria.fr/~fpottier/menhir/[menhir] instead, which
has tons of extra features, lets you write more readable grammars, and
has excellent documentation.

=== Complete file listing

  xref:BOOTSTRAP.adoc[]::        instructions for bootstrapping
  link:Changes[]::               what’s new with each release
  link:CONTRIBUTING.md[]::       how to contribute to OCaml
  xref:HACKING.adoc[]::          this file
  xref:INSTALL.adoc[]::          instructions for installation
  link:LICENSE[]::               license and copyright notice
  link:Makefile[]::              main Makefile
  link:Makefile.common[]::       common Makefile definitions
  xref:README.adoc[]::           general information on the compiler distribution
  xref:README.win32.adoc[]::     general information on the Windows ports of OCaml
  link:VERSION[]::               version string. Run `make configure` after changing.
  link:asmcomp/[]::              native-code compiler and linker
  link:boot/[]::                 bootstrap compiler
  link:build-aux/[]::            autotools support scripts
  link:bytecomp/[]::             bytecode compiler and linker
  link:compilerlibs/[]::         the OCaml compiler as a library
  link:configure[]::             configure script
  link:configure.ac[]::          autoconf input file
  link:debugger/[]::             source-level replay debugger
  link:driver/[]::               driver code for the compilers
  link:flexdll/[]::              git submodule -- see xref:README.win32.adoc[]
  link:lex/[]::                  lexer generator
  link:man/[]::                  man pages
  link:manual/[]::               system to generate the manual
  link:middle_end/[]::           the flambda optimisation phase
  link:ocamldoc/[]::             documentation generator
  link:ocamltest/[]::            test driver
  link:otherlibs/[]::            several additional libraries
  link:parsing/[]::              syntax analysis -- see xref:parsing/HACKING.adoc[]
  link:release-info/[]::         documentation and tools to prepare releases
  link:runtime/[]::              bytecode interpreter and runtime systems
  link:stdlib/[]::               standard library
  link:testsuite/[]::            tests -- see xref:testsuite/HACKING.adoc[]
  link:tools/[]::                various utilities
...
Besides the targets listed in xref:INSTALL.adoc[] for build and
...
`make -C testsuite parallel`:: see xref:testsuite/HACKING.adoc[]
...
You only need to run `configure` once, but you will need to run
`make clean-for-dune` every time you want to run `dune` after you built
...
file. Using a Docker image with GNU Autoconf installed, and
`git rebase --exec`, one can properly regenerate the `configure`
script for each commit in a branch.
...
(hopefully to add a new entry). In general all patches submitted should come
with a Changes entry; see the guidelines in link:CONTRIBUTING.md[].
...
- `run-multicoretests`: run an extensive testsuite covering multicore
  features.
