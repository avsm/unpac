...
let lexer_loc l = Toml_error.loc ~file:l.file ~line:l.line ~column:l.col ()
...
let validate_utf8_at_pos_bytes l =
  if l.pos >= l.input_len then
    Toml_error.raise_lexer ~location:(lexer_loc l) Unexpected_eof;
  let byte_len = utf8_byte_length_from_first_byte (Bytes.unsafe_get l.input l.pos) in
  if byte_len = 0 then
    Toml_error.raise_lexer ~location:(lexer_loc l) Invalid_utf8;
  if l.pos + byte_len > l.input_len then
    Toml_error.raise_lexer ~location:(lexer_loc l) Incomplete_utf8;
    ...
    while !continue && not (is_eof l) && get_current l <> '\n' do
      ...
      if c = '\r' then begin
        ...
        if l.pos + 1 < l.input_len && get_char l (l.pos + 1) = '\n' then
          ...
          Toml_error.raise_lexer ~location:(lexer_loc l) Bare_carriage_return
        ...
        if code < 0x09 || (code > 0x09 && code < 0x20) || code = 0x7F then
          Toml_error.raise_lexer ~location:(lexer_loc l) (Control_character code);
        advance l
      end
    done
  end
...
let hex_value c =
  ...
  else Toml_error.raise_number Invalid_hex_digit
...
let unicode_to_utf8 l codepoint =
  if codepoint < 0 || codepoint > 0x10FFFF then
    Toml_error.raise_lexer ~location:(lexer_loc l) (Invalid_unicode_codepoint codepoint);
  if codepoint >= 0xD800 && codepoint <= 0xDFFF then
    Toml_error.raise_lexer ~location:(lexer_loc l) (Surrogate_codepoint codepoint);
  let buf = Buffer.create 4 in
  Uutf.Buffer.add_utf_8 buf (Uchar.of_int codepoint);
  Buffer.contents buf
...
let parse_escape l =
  ...
  | 'x' ->
      ...
      if l.pos + 1 >= l.input_len then
        Toml_error.raise_lexer ~location:(lexer_loc l) (Incomplete_escape "\\x");
      let c1 = get_char l l.pos in
      let c2 = get_char l (l.pos + 1) in
      if not (is_hex_digit c1 && is_hex_digit c2) then
        Toml_error.raise_lexer ~location:(lexer_loc l) (Invalid_unicode_escape "\\x");
  ...
  | 'u' ->
      ...
      if l.pos + 3 >= l.input_len then
        Toml_error.raise_lexer ~location:(lexer_loc l) (Incomplete_escape "\\u");
      let s = sub_string l l.pos 4 in
      for i = 0 to 3 do
        if not (is_hex_digit s.[i]) then
          Toml_error.raise_lexer ~location:(lexer_loc l) (Invalid_unicode_escape "\\u")
      done;
  ...
  | 'U' ->
      ...
      unicode_to_utf8 l cp
  | _ ->
      Toml_error.raise_lexer ~location:(lexer_loc l) (Invalid_escape c)
...
let of_string input =
  try
    Ok (parse_toml input)
  with
  | Failure msg -> Error (Toml_error.make (Toml_error.Syntax (Toml_error.Expected msg)))
  | Toml_error.Error e -> Error e
  | e -> Error (Toml_error.make (Toml_error.Syntax (Toml_error.Expected (Printexc.to_string e))))
...
(* ============================================
   Tagged JSON (toml-test interoperability)
   ============================================ *)
