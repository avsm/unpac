...
module Update = struct
  ...
  let _destination_of_string = function
    ...
    | s ->
        raise
          (Invalid_argument
             (Printf.sprintf "destination_of_string: unknown %s" s))
  ...
  let _update_type_of_string = function
    ...
    | s ->
        raise
          (Invalid_argument
             (Printf.sprintf "update_type_of_string: unknown %s" s))
...
(** Default callbacks *)
let default_allow_callback ~tool_name:_ ~input:_ ~context:_ = Result.allow ()

let discovery_callback log ~tool_name:_ ~input:_ ~context =
  List.iter
    (fun update ->
      match Update.rules update with
      | Some rules -> List.iter (fun rule -> log := rule :: !log) rules
      | None -> ())
